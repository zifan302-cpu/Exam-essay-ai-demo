...
export async function generatePostgraduateEssay(input, env = {}) {
  const apiKey = getEnv(env, "DASHSCOPE_API_KEY");
  if (!apiKey) {
  ...
  if (!input?.topic?.trim()) {
    throw new HttpError(400, { error: "missing_topic" });
  }

  const model = getEnv(env, "DASHSCOPE_MODEL") || DEFAULT_MODEL;
  ...
  return normalizeResult(parsed, input, model, payload.usage);
}
